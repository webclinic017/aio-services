...
Aio Services can automatically generate [Async API](https://www.asyncapi.com/) documentation of your service.

### Usage

```shell
asvc generate-docs {package}.{module}:{service-instance} --format=yaml|json --out=output-path
```
Example:
```shell
asvc generate-docs examples.asyncapi:service --format=yaml --out=./asyncapi.yaml
```
